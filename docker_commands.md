...
```bash
docker run -d -p 8000:8000 -v fastapi-db:/app/db --name sever1 shopping-server1:v1
```
...
```bash
docker run -d -p 8001:8000 -v fastapi-db:/app/db --name sever2 shopping-server2:v1
